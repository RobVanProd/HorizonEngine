...
import { sampleHeightWorld, type Heightfield } from '@engine/world';
import { QuestChainManager, type QuestJournalState } from './quest-system.js';

const COLLECT_RADIUS = 4.75;
const LANDMARK_RADIUS = 7;
const QUEST_CHAIN_TITLE = 'Echoes In The Hollow';

export interface GameQuestAnchors {
  trailhead: [number, number, number];
  camp: [number, number, number];
  shrine: [number, number, number];
...
export interface GameDemoState {
  ...
  total: number;
  complete: boolean;
  quest: QuestJournalState;
}

export interface GameDemoOptions {
  bounds: { min: [number, number, number]; max: [number, number, number]; center: [number, number, number] };
  heightfield?: Heightfield | null;
  /** Alternative to heightfield: sample ground height at (x,z). Used for streaming. */
  groundSampler?: (x: number, z: number) => number;
  /** Optional authored/procedural route points for collectible placement. */
  interestPoints?: Array<[number, number, number]>;
  levelName?: string;
...
export class GameDemo {
  ...
  private readonly _collectibleIds = new Set<number>();
  private _collected = 0;
  private _total = 0;
  private _complete = false;
  private _levelName = 'Playable Slice';
  private _collectibleMeshHandle = 0;
  private _collectibleMaterialHandle = 0;
  private _ringMeshHandle = 0;
  ...
  spawn(options: GameDemoOptions): void {
    ...
    const sampleHeight = groundSampler ?? (heightfield ? (x: number, z: number) => sampleHeightWorld(heightfield, x, z) : () => 0);

    this._collectibleMeshHandle = this._engine.registerMesh(
      GPUMesh.create(device, createSphere(0.55, 20, 16)),
    );
    this._ringMeshHandle = this._engine.registerMesh(
      GPUMesh.create(device, createTorus(1.55, 0.14, 42, 14)),
    );
  ...
  update(playerEye: [number, number, number]): GameDemoState {
    const world = this._engine.world;
    const quest = this._quest;
    const anchors = this._anchors;

    if (!quest || !anchors) return this.getState();
  ...
  private activateSpringEmitter(): void {
    if (this._springEmitterId === 0 || !this._engine.world.has(this._springEmitterId)) return;
    this._engine.world.setField(
      this._springEmitterId,
      ParticleEmitter,
      'flags',
...
export function createGameHud(): {
  root: HTMLDivElement;
  update: (state: GameDemoState) => void;
  show: () => void;
  hide: () => void;
} {
...
function resolveQuestAnchors(
  bounds: GameDemoOptions['bounds'],
  route: Array<[number, number, number]>,
  explicit?: Partial<GameQuestAnchors>,
): GameQuestAnchors {
...
function pickCollectiblePositions(route: Array<[number, number, number]>, anchors: GameQuestAnchors): Array<[number, number, number]> {
  if (route.length >= 5) {
...
function buildFallbackRoute(bounds: GameDemoOptions['bounds']): Array<[number, number, number]> {
