...
- The AI/editor integration now exposes `editor.captureViewport` for direct viewport PNG capture and `scene.layoutSummary` for a compact top-down occupancy/landmark summary, which gives engine-facing agents much better scene context than raw entity lists alone.
