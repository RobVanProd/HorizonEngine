...
export function registerEditorCommands(router, editor) {
    ...
    router.register({
        ...
        if (!valid.includes(tool))
            return { ok: false, error: `Invalid tool. Use: ${valid.join(', ')}` };
        editor.toolbar.setTool(tool);
    ...
    router.register({
        ...
        if (params['targetX'] !== undefined)
            cam.target[0] = Number(params['targetX']);
        if (params['targetY'] !== undefined)
            cam.target[1] = Number(params['targetY']);
        if (params['targetZ'] !== undefined)
            cam.target[2] = Number(params['targetZ']);
        if (params['distance'] !== undefined)
            cam.distance = Number(params['distance']);
        if (params['yaw'] !== undefined)
            cam.yaw = Number(params['yaw']);
        if (params['pitch'] !== undefined)
            cam.pitch = Number(params['pitch']);
        return { ok: true };
    });
    router.register({
        ...
        if (!valid.includes(preset))
            return { ok: false, error: `Invalid preset. Use: ${valid.join(', ')}` };
        editor.viewport.camera.setPreset(preset);
        return { ok: true };
    });
    router.register({
    ...
    router.register({
        ...
        if (!valid.includes(type))
            return { ok: false, error: `Invalid type. Use: ${valid.join(', ')}` };
        // Trigger the editor's add primitive flow
        editor._addPrimitive(type);
        return { ok: true };
    });
    router.register({
        ...
        const entityId = await editor._spawnProceduralTree(preset, { seed, scale });
    ...
    router.register({
        ...
        const hasTarget = params['targetX'] !== undefined ||
        ...
        return {
            ...
            data: await editor.viewport.captureSnapshotWithOptions({
                preset: params['preset'] !== undefined ? String(params['preset']) : undefined,
    ...
    router.register({
        ...
        const overlay = String(params['overlay'] ?? '');
    ...
    router.register({
        ...
        await editor.viewport.pickScreenPoint(Number(params['x']), Number(params['y']), Boolean(params['additive'] ?? false));
