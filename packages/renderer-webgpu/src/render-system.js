 ...
 */
export function createRenderSystem(world, ctx) {
  ...
  const grassQuery = world.query(WorldMatrix, MeshRef, GrassRef, Visible);
    function render(_fc) {
        ...
        query.each((arch, count) => {
            ...
            const entityIds = arch.entities.data;
...
function isMeshVisible(model, boundsMin, boundsMax, radius, planes) {
    ...
    for (let i = 0; i < 6; i++) {
        const offset = i * 4;
        const distance = planes[offset] * cx +
            planes[offset + 1] * cy +
            planes[offset + 2] * cz +
            planes[offset + 3];
...
function extractFrustumPlanes(vp, out) {
...
function setPlane(out, offset, a, b, c, d) {
