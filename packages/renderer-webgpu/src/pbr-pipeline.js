...
export class PBRRenderer {
    ...
    get gpuProfiler() { return this._gpuProfiler; }
    get frameStats() { return this._frameStats; }
    ...
    async initialize(envConfig, shadowConfig) {
    ...
    setLighting(lighting) {
        ...
        d[2] = dz / len;
        d[3] = 0;
        d[4] = lighting.color[0];
        d[5] = lighting.color[1];
        d[6] = lighting.color[2];
        ...
        this._gpu.device.queue.writeBuffer(this._lightBuffer, 0, d);
    }
    beginFrame() {
    ...
    drawMesh(mesh, material, modelMatrix) {
        if (this._draws.length >= MAX_OBJECTS)
            return;
        ...
        data.set(modelMatrix, 0);
        computeNormalMatrix(modelMatrix, data, 16);
        this._gpu.device.queue.writeBuffer(this._objectBuffers[idx], 0, data);
    }
    /**
     * Draw a skinned mesh with joint matrices.
     * jointMatrices: flat Float32Array of mat4x4f per joint (joint_count * 16 floats).
     */
    drawSkinnedMesh(mesh, material, modelMatrix, jointMatrices) {
        if (this._draws.length >= MAX_OBJECTS)
            return;
        ...
        this._gpu.device.queue.writeBuffer(this._jointBuffers[idx], 0, jointMatrices.buffer, jointMatrices.byteOffset, len * 4);
    }
    recordCulledMesh(mesh) {
        this._frameStats.culledObjects++;
        this._frameStats.culledTriangles += mesh.triangleCount;
    }
    endFrame(afterMainPass) {
        ...
        if (this._profilingEnabled && this._gpuProfiler) {
            ...
            if (tw)
                mainPassDesc.timestampWrites = tw;
        }
    ...
    handleResize() { this._createDepthTexture(); }
    destroy() {
...
function computeNormalMatrix(m, out, offset) {
    ...
    if (Math.abs(det) < 1e-8)
        det = 1;
    const id = 1 / det;
    out[offset + 0] = (a11 * b11 - a12 * b10 + a13 * b09) * id;
    out[offset + 1] = (a12 * b08 - a10 * b11 - a13 * b07) * id;
    out[offset + 2] = (a10 * b10 - a11 * b08 + a13 * b06) * id;
    out[offset + 3] = 0;
    out[offset + 4] = (a02 * b10 - a01 * b11 - a03 * b09) * id;
    out[offset + 5] = (a00 * b11 - a02 * b08 + a03 * b07) * id;
    out[offset + 6] = (a01 * b08 - a00 * b10 - a03 * b06) * id;
    out[offset + 7] = 0;
    out[offset + 8] = (a31 * b05 - a32 * b04 + a33 * b03) * id;
    out[offset + 9] = (a32 * b02 - a30 * b05 - a33 * b01) * id;
    out[offset + 10] = (a30 * b04 - a31 * b02 + a33 * b00) * id;
    out[offset + 11] = 0;
    out[offset + 12] = 0;
    out[offset + 13] = 0;
    out[offset + 14] = 0;
...
function debugViewToIndex(view) {
    switch (view) {
