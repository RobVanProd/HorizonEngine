...
export class Environment {
    ...
    maxMipLevel;
    sampler;
    constructor(envCubemap, skyboxCubemap, prefilteredMap, irradianceMap, brdfLUT, maxMipLevel, sampler) {
    ...
    static async generate(device, config = {}) {
    ...
    destroy() {
        ...
        if (this.skyboxCubemap !== this.envCubemap)
...
async function verifyTexture(device, texture, size, label) {
    ...
    encoder.copyTextureToBuffer({ texture, origin: { x: 0, y: size >> 1, z: 0 } }, { buffer: staging, bytesPerRow: alignedRowBytes }, { width: size, height: 1, depthOrArrayLayers: 1 });
...
async function verifyTexture2D(device, texture, size, label) {
...
async function runComputeWithErrorCheck(device, label, pipeline, bindGroup, workgroups) {
    ...
    if (err)
        console.error(`[IBL] ${label} GPU ERROR:`, err.message);
    else
        console.log(`[IBL] ${label} OK`);
}
async function generateProceduralSky(device, target, size, sunDir, sunIntensity) {
    ...
    device.queue.writeBuffer(paramsBuffer, 16, paramsU32);
    ...
    await runComputeWithErrorCheck(device, 'genCubemap', pipeline, bindGroup, [Math.ceil(size / 8), Math.ceil(size / 8), 6]);
    paramsBuffer.destroy();
}
async function generateCubemapMipmaps(device, cubemap, size, mipLevels) {
...
async function prefilterSpecular(device, source, target, baseSize, mipLevels, sampleCount, sampler) {
    ...
    for (let mip = 0; mip < mipLevels; mip++) {
        ...
        device.queue.writeBuffer(paramsBuffer, 0, paramsF32);
        ...
        await runComputeWithErrorCheck(device, `prefilter-mip${mip}`, pipeline, bindGroup, [Math.ceil(mipSize / 8), Math.ceil(mipSize / 8), 6]);
    }
    paramsBuffer.destroy();
}
async function computeIrradiance(device, source, target, size, sampler) {
    ...
    await runComputeWithErrorCheck(device, 'irradiance', pipeline, bindGroup, [Math.ceil(size / 8), Math.ceil(size / 8), 6]);
    paramsBuffer.destroy();
}
async function computeBrdfLUT(device, target, size, sampleCount) {
    ...
    await runComputeWithErrorCheck(device, 'brdfLUT', pipeline, bindGroup, [Math.ceil(size / 8), Math.ceil(size / 8)]);
    paramsBuffer.destroy();
}
async function convertEquirectToCubemap(device, hdr, target, cubemapSize, sampler) {
    ...
    device.queue.writeTexture({ texture: equirectTex }, rgba, { bytesPerRow: hdr.width * 16 }, { width: hdr.width, height: hdr.height });
    ...
    await runComputeWithErrorCheck(device, 'equirectToCube', pipeline, bindGroup, [Math.ceil(cubemapSize / 8), Math.ceil(cubemapSize / 8), 6]);
    paramsBuffer.destroy();
    equirectTex.destroy();
}
