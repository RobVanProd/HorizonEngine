import { LocalTransform, Parent, Visible, WorldMatrix, } from '@engine/ecs';
import { buildDiffFromSnapshots, } from './plan-diff.js';
import { buildPredictedCreatedSnapshot, normalizeTransform, sanitizeName, snapshotEntity, withRenamedSnapshot, withTransformedSnapshot, } from './entity-snapshots.js';
import { normalizePlan, validatePlan, } from './plan-validator.js';
export function previewAiActionPlan(engine, planInput, options = {}) {
...
export function applyAiActionPlan(engine, planInput, options = {}) {
    return applyAiActionPlanWithUndoLog(engine, planInput, options).publicResult;
}
export function applyAiActionPlanWithUndoLog(engine, planInput, options = {}) {
    const preview = previewAiActionPlan(engine, planInput, {
...
export function undoAiActionPlanExecution(execution) {
    for (let i = execution.undoSteps.length - 1; i >= 0; i--) {
        execution.undoSteps[i].undo();
    }
}
function buildPreviewSnapshots(engine, plan) {
    const snapshots = [];
    for (const action of plan.actions) {
        switch (action.kind) {
            case 'entity.create': {
                const predicted = buildPredictedCreatedSnapshot(action.payload);
                snapshots.push({
                    actionId: action.actionId,
                    before: null,
                    after: predicted,
            ...
            case 'entity.rename': {
                ...
                snapshots.push({
                    ...
                    before,
                    after: withRenamedSnapshot(before, action.payload.name),
                });
                break;
            }
            case 'entity.setTransform': {
                ...
                snapshots.push({
                    ...
                    entityId: action.entityId,
                    before,
                    after: withTransformedSnapshot(before, action.payload),
                });
                break;
            }
        }
...
function createEntity(engine, payload) {
...
function applyTransform(engine, entityId, payload) {
    const world = engine.world;
    if (payload.position) {
