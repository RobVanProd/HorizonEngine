...
export interface ControlPlaneRegistrationOptions {
  undoBridge?: AiPlanUndoBridge;
}

export function registerControlPlaneCommands(
  router: CommandRouter,
  engine: Engine,
  options: ControlPlaneRegistrationOptions = {},
): void {
  router.register(
    {
    ...
    (params) => {
      ...
      const entityIds = Array.isArray(params['entityIds'])
        ? (params['entityIds'] as unknown[]).map((value) => Number(value))
        : undefined;
      return {
        ok: true,
  ...
  router.register(
    ...
    (params) => {
      const plan = (params['plan'] ?? null) as AiActionPlan | null;
      return {
        ok: true,
