export function buildDiffFromSnapshots(entries) {
...
function diffSnapshotFields(before, after) {
    const changed = [];
    if (before.name !== after.name)
        changed.push('name');
    if (before.parentId !== after.parentId)
        changed.push('parentId');
    if (!equalArrays(before.tags, after.tags))
        changed.push('tags');
    if (!equalArrays(before.components, after.components))
        changed.push('components');
    if (!equalTransform(before.transform, after.transform))
